...
useEffect(() => {
  ...
  const handleMessageEvent = (event) => {
    ...
    if (messageData.type === 'messages') {
      ...
      const messages = dispatch(fetchChatSuccess((prevMessage)=>[...prevMessage,messageData])); // Dispatch new Message what i have sent 
      setAllMessages(messages.payload); // Dispatch action to update messages in the state
  ...
  return (
    ...
    <>
      ...
      <Sidebar latestMsg={recentMessage?.length > 0 ? recentMessage[recentMessage?.length - 1]?.message : " "} time={recentMessage?.length > 0 ? recentMessage[recentMessage?.length - 1]?.createdAt : " "} />
